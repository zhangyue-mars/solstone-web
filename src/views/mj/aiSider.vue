...
const UserAgreementModal = defineAsyncComponent(
	() => import("./components/UserAgreementModal.vue")
);
const ActivateCodeModal = defineAsyncComponent(
	() => import("./components/ActivateCodeModal.vue")
);
...
const showActivateCodeModal = ref(false);
...
const menuOptions = ref([
	...
	{
		label: "凭码兑换",
		key: "activateCode", // 或者 'activateCode'
		icon: renderIcon(key), // 这里的图标可以换成合适的
	},
	{
		label: "退出账号",
		key: "logout",
...
const handleSelect = (key: string) => {
	...
	// } else if (key === "buy") {
	// 	show.value = true;
	} else if (key === "activateCode") {
		showActivateCodeModal.value = true;
	}
};
</script>
<template>
	<div
		class="flex-shrink-0 w-[70px] z-[1000] h-full"
		v-if="!isMobile"
  ...
  <!-- 凭码兑换对话框 -->
  <ActivateCodeModal v-model:visible="showActivateCodeModal" />
</template>
