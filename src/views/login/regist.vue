...
const codeButtonText = computed(() => {
	return isSending.value
		? `${countdown.value}s`
		: t("register.get_verification_code");
});
...
async function sendVerificationCode() {
	...
	try {
		...
		await getVerificationCode(registerForm.email); // 使用邮箱
...
const brandSectionStyle = computed(() => {
	const isDark =
		themeVars.value.bodyColor.startsWith("#1") ||
					...
					<div class="form-content">
						...
						<div class="input-group">
							<div class="input-label-row">
								<NText strong class="input-label">{{ $t("邮箱（登录账号）") }}</NText>
